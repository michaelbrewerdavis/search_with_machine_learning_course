...
def extract_logged_features(hits, query_id):
    ...
    for (idx, hit) in enumerate(hits):
        # print(hit['fields']['_ltrlog'][0]['ltr_log'])
        ...
        feature_log = hit['fields']['_ltrlog'][0]['ltr_log']
        for k in FEATURE_KEYS:
            feature_record = next(filter(lambda v: v['name'] == k, feature_log))
            value = feature_record.get('value', 0)
            feature_results[k].append(value)
